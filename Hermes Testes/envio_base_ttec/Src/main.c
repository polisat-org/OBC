...
/* USER CODE BEGIN PV */
telemetryFull t1, t2;
uint8_t buffer_envio[51];

uint8_t tamanho1, tamanho2;
...
int main(void)
{
  ...
  while (1)
  {
	...
	tamanho1 = readyData(&t1, buffer_envio);
    //printf("Mandando buffer: \n\r");
    tamanho2 = parseData(&t2, buffer_envio);

    if(t1.segundo % 15 == 0)	printTelemetry(&t2);
